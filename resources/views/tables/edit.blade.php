<x-app-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h4 class="text-xl font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">Edit Meja</h4>
            <p class="text-sm font-medium text-zinc-500 mt-1">Ubah data meja self-order</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-zinc-100 dark:border-zinc-800 overflow-hidden w-full">
        <form action="{{ route('tables.update', $table->id) }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <x-form-label for="table_number">Nomor Meja</x-form-label>
                <x-form-input type="text" id="table_number" name="table_number" value="{{ old('table_number', $table->table_number) }}" 
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none {{ $errors->has('status') ? 'border-rose-300 focus:ring-rose-500/20 focus:border-rose-500' : '' }}">
                    <option value="active" {{ old('status', $table->status) === 'active' ? 'selected' : '' }}>Aktif</option>
                    <option value="inactive" {{ old('status', $table->status) === 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @error('status')
                    <p class="text-sm text-rose-500 mt-1.5 font-medium label-error">{{ $message }}</p>
                @enderror
