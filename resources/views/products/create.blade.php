<x-app-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h4 class="text-xl font-bold text-gray-900 tracking-tight">Tambah Produk</h4>
            <p class="text-sm font-medium text-gray-500 mt-1">Tambahkan produk baru ke dalam inventaris</p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden w-full">
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" x-data="{ submitting: false }" @submit="submitting = true" class="p-6 md:p-8 space-y-6">
            ...
            <div x-data="{
                selectedGroups: {{ json_encode(old('modifier_groups', [])) }},
            ...
            <div>
                ...
                <div class="mt-1 flex items-center justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-lg transition-colors duration-200 hover:border-[#C4C3C0] bg-gray-50">
                    <div class="space-y-1 text-center">
                        ...
                        <p class="text-xs text-gray-400 mt-2" id="file-name">PNG, JPG hingga 2MB</p>
                    </div>
                </div>
                @error('product_photo')
                    <p class="text-sm text-rose-500 mt-1.5 font-medium">{{ $message }}</p>
