<div x-data @keydown.escape.window="$wire.closeDetail()">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
        <div class="card-surface p-6">
            ...
            <h3 class="text-2xl font-extrabold text-[#37352F] tracking-tight">{{ number_format($this->summary->total_count) }}</h3>
        </div>
        <div class="card-surface p-6">
            <div class="flex justify-between items-start mb-3">
            ...
            <h3 class="text-2xl font-extrabold text-[#37352F] tracking-tight">Rp {{ number_format($this->summary->total_paid_amount, 0, ',', '.') }}</h3>
        </div>
    </div>
    ...
    <div class="card-surface p-4 lg:p-5 mb-6 flex flex-col gap-4">
        <div class="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
            <div class="relative w-full lg:max-w-xs">
                <span class="material-symbols-rounded absolute left-3 top-1/2 -translate-y-1/2 text-[#9B9A97] text-[20px]">search</span>
                <input type="search" wire:model.live.debounce.400ms="search"
                       placeholder="Cari kode pesanan (POS-XXXXXX)..."
            ...
            <div wire:loading.flex wire:target="search,statusFilter,datePreset,startDate,endDate" class="hidden items-center gap-2 text-xs text-[#9B9A97] shrink-0">
                <span class="h-3.5 w-3.5 rounded-full border-2 border-[#37352F]/30 border-t-[#37352F] animate-spin"></span> Memuat...
            </div>
            ...
            <table class="data-table relative">
                <thead class="sticky top-0 z-10 shadow-sm">
                    <tr class="bg-[#F7F7F5]">
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-[#787774] uppercase tracking-wider">Kasir &amp; Tipe</th>
                ...
                <tbody class="divide-y divide-[#E9E9E7]">
                    @forelse ($this->orders as $order)
                        <tr wire:key="order-{{ $order->id }}" class="hover:bg-[#F7F7F5] transition-colors duration-200">
                            <td class="px-6 py-4">
                                <div class="font-mono font-bold text-[#37352F] text-sm">{{ $order->order_code }}</div>
                                <div class="text-xs text-[#9B9A97] mt-0.5">{{ $order->created_at->format('d M Y, H:i') }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-[#37352F] max-w-[220px] truncate">
                                    ...
                                    @if ($order->order_items_count > 2)
                                        <span class="text-[#9B9A97]">+{{ $order->order_items_count - 2 }} lainnya</span>
                                    @endif
                                </div>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center gap-1 text-xs font-semibold text-[#37352F] bg-[#F1F1EF] border border-[#E9E9E7] rounded-lg px-2 py-1 whitespace-nowrap">
                                    <span class="material-symbols-rounded text-[13px]">{{ $order->payment_method?->value === 'cash' ? 'payments' : 'qr_code_2' }}</span>
                                    {{ $order->payment_method?->label() ?? '-' }}
                                </span>
                                @if ($order->payments->count() > 1)
                                    <span class="block text-[10px] font-bold text-blue-600 mt-1 uppercase tracking-wider">Split {{ $order->payments->count() }} Metode</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <span class="text-sm font-bold text-[#37352F] tabular-nums">Rp {{ number_format($order->order_amount, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    @if (in_array($order->order_status->value, ['paid', 'completed'], true))
                                        <a href="{{ route('transaction.receipt', $order->order_code) }}" target="_blank" rel="noopener"
    ...
    @if ($this->selectedOrder)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-[#37352F]/40 backdrop-blur-sm p-4" wire:click.self="closeDetail">
            <div class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl border border-[#E9E9E7]">
                ...
                <div class="px-6 py-4 border-b border-[#E9E9E7] flex justify-between items-start shrink-0">
                    <div>
                        ...
                        <p class="text-sm text-[#787774] mt-0.5">{{ $detail->created_at->format('d F Y, H:i') }} WIB</p>
                    </div>
                ...
                <div class="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6">
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        @if ($detail->voided_at)
                            <div>
                                <p class="text-xs font-semibold text-rose-500 uppercase tracking-wider mb-1">Dibatalkan (Void)</p>
                                <p class="font-medium text-rose-600 text-xs">oleh {{ $detail->voidedBy->name ?? '-' }} · {{ $detail->voided_at->format('d M Y H:i') }}</p>
                    ...
                    @if ($detail->customer)
                        <div class="bg-[#F7F7F5] rounded-xl p-4 border border-[#E9E9E7]">
                            <p class="text-xs font-bold text-[#9B9A97] uppercase tracking-wider mb-2">Pelanggan &amp; Loyalty</p>
                            <div class="flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="font-semibold text-sm text-[#37352F] truncate">{{ $detail->customer->name }}</p>
                                    <p class="text-xs text-[#787774]">{{ $detail->customer->phone ?? '-' }}</p>
                                </div>
                                @if ($detail->customer->loyaltyAccount)
                                    <div class="text-right shrink-0">
                                        <p class="text-xs font-semibold text-[#787774]">{{ $detail->customer->loyaltyAccount->currentTier->name ?? 'Reguler' }}</p>
                                        <p class="text-sm font-bold text-[#37352F]">{{ number_format($detail->customer->loyaltyAccount->current_points ?? 0) }} poin</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <ul class="divide-y divide-[#E9E9E7] border border-[#E9E9E7] rounded-xl overflow-hidden">
                            @foreach ($detail->orderItems as $item)
                                <li class="p-3 flex justify-between items-start gap-3 bg-white">
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-[#37352F]">{{ $item->qty }}× {{ $item->product->product_name ?? 'Produk Dihapus' }}</p>
                                        @if (! empty($item->options))
                                            <p class="text-xs text-[#787774] mt-0.5">{{ collect($item->options)->pluck('name')->filter()->implode(', ') }}</p>
                                        @endif
                                        @if ($item->notes)
                                            <p class="text-xs italic text-[#9B9A97] mt-0.5">Catatan: {{ $item->notes }}</p>
                                        @endif
                                    </div>
                                    <span class="text-sm font-bold text-[#37352F] tabular-nums shrink-0">Rp {{ number_format($item->order_subtotal, 0, ',', '.') }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div>
                        <p class="text-xs font-bold text-[#9B9A97] uppercase tracking-wider mb-2">Rincian Pembayaran</p>
                        @if ($detail->payments->isNotEmpty())
                            <ul class="space-y-2">
                                @foreach ($detail->payments as $payment)
                                    <li class="flex items-center justify-between bg-white border border-[#E9E9E7] rounded-xl px-4 py-3 gap-3">
                                        <div class="flex items-center gap-2 min-w-0">
                                            <span class="material-symbols-rounded text-[18px] text-[#787774] shrink-0">{{ $payment->payment_method->value === 'cash' ? 'payments' : 'credit_card' }}</span>
                                            <div class="min-w-0">
                                                <p class="text-sm font-semibold text-[#37352F]">{{ $payment->payment_method->label() }}</p>
                                                @if ($payment->reference_number)
                                                    <p class="text-[11px] text-[#9B9A97] font-mono truncate">Ref: {{ $payment->reference_number }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="text-right shrink-0">
                                            <p class="text-sm font-bold text-[#37352F] tabular-nums">Rp {{ number_format((float) $payment->amount, 0, ',', '.') }}</p>
                                            <p class="text-[11px] font-medium {{ $payment->status->value === 'captured' ? 'text-emerald-600' : 'text-[#9B9A97]' }}">{{ $payment->status->label() }}</p>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
                        @else
                            <p class="text-sm text-[#9B9A97] italic">Belum ada catatan pembayaran granular untuk pesanan ini.</p>
                        @endif
                    </div>

                    <div class="bg-[#F7F7F5] rounded-xl p-4 border border-[#E9E9E7] text-sm space-y-1.5">
                        <div class="flex justify-between text-[#787774]"><span>Subtotal</span><span class="tabular-nums">Rp {{ number_format($detail->subtotal_amount, 0, ',', '.') }}</span></div>
