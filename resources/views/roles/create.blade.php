<x-app-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm font-medium text-[#787774] mt-1">Tambahkan peran pengguna baru</p>
        </div>
        ...
        <form action="{{ route('roles.store') }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true" class="p-6 md:p-8 space-y-6">
            ...
            <div>
                ...
                <x-form-input type="text" id="name" name="name" value="{{ old('name') }}" 
                       placeholder="Masukkan nama peran (misal Administrator)" required class="{{ $errors->has('name') ? 'input-error' : '' }}" />
                @error('name')
