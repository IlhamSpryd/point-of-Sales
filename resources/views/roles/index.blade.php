                    ...
                    @forelse($roles as $role)
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($role->description)
                                <div class="text-xs text-zinc-500 mt-1 truncate max-w-xs" title="{{ $role->description }}">{{ $role->description }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-zinc-500">{{ $role->created_at?->format('M d, Y') ?? '—' }}</td>
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="{{ route('roles.edit', $role->id) }}" class="p-1.5 text-zinc-400 hover:text-blue-600 transition-colors inline-block"><i class="bi bi-pencil-square"></i></a>
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-zinc-500 text-sm">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="bi bi-shield-lock text-4xl mb-3 text-zinc-400"></i>
                                    <p>No roles found.</p>
                                </div>
                            </td>
                        </tr>
