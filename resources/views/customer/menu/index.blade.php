...
<div class="mx-auto min-h-screen max-w-md pb-32 sm:border-x sm:border-yovel-border"
    ...
    <header class="sticky top-0 z-40 h-14 flex items-center justify-between gap-3 px-4 bg-yovel-bg/90 backdrop-blur-xl border-b border-yovel-border">
    ...
    <div class="px-4 pt-5 pb-2 space-y-4">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-yovel-ink text-white p-6 shadow-md mb-4">
            <div class="relative z-10">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/20 backdrop-blur-md text-[10px] font-bold uppercase tracking-wider mb-3 shadow-sm border border-white/10">
            ...
            <div class="absolute -top-10 -right-5 w-24 h-24 bg-amber-500/20 rounded-full blur-xl pointer-events-none"></div>
        </div>
    ...
    {{-- Kategori Dinamis --}}
    <nav aria-label="Kategori menu" class="sticky top-14 z-30 bg-yovel-bg/95 backdrop-blur-md pb-2 pt-1 border-b border-yovel-border/60 shadow-sm">
        <div class="flex gap-2.5 overflow-x-auto scrollbar-hide px-4 py-2 items-center">
            <button type="button" @click="activeCategory = 'all'"
                    ...
                    :class="activeCategory === 'all' ? 'bg-[#37352F] text-white border-[#37352F] shadow-md' : 'bg-white text-gray-500 border-[#E9E9E7] hover:bg-gray-50'"
            ...
            @foreach ($visibleCategories as $cat)
                <button type="button" @click="activeCategory = '{{ $cat->id }}'"
                        ...
                        :class="activeCategory === '{{ $cat->id }}' ? 'bg-[#37352F] text-white border-[#37352F] shadow-md' : 'bg-white text-gray-500 border-[#E9E9E7] hover:bg-gray-50'"
                        class="shrink-0 flex items-center justify-center h-11 px-5 rounded-full border text-sm font-semibold whitespace-nowrap transition-all duration-200 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#37352F]">
                    {{ $cat->category_name }}
                </button>
            @endforeach
    ...
    <main class="px-4 pt-4">
        ...
        @else
                        ...
                        <button type="button"
                            ...
                            <span class="relative mb-3 block aspect-square overflow-hidden rounded-2xl bg-yovel-bg">
                                @if ($product->product_photo)
                                    <img src="{{ asset('storage/'.$product->product_photo) }}" alt="" loading="lazy"
                                ...
                                @else
                                    <span class="flex h-full w-full items-center justify-center">
                                        <span class="material-symbols-rounded text-[48px] text-primary-300">coffee</span>
                                    </span>
                                @endif
                                @if ($soldOut)
                                    <span class="absolute left-2 top-2 rounded-lg bg-yovel-ink px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-white backdrop-blur-md">Habis</span>
                                @endif
                            </span>
                            
                            {{-- Nama Produk --}}
                            <span class="mb-1 line-clamp-2 text-sm font-bold leading-tight {{ $soldOut ? 'text-yovel-muted' : '' }}">{{ $product->product_name }}</span>
                            
                            {{-- Deskripsi Singkat --}}
                            <span class="mb-3 line-clamp-2 text-[10px] leading-snug text-gray-500 font-medium">{{ $desc }}</span>
                            
                            {{-- Harga & Tombol --}}
                            <span class="mt-auto flex items-center justify-between pt-1 w-full">
                                <span class="text-sm font-bold tracking-tight tabular-nums {{ $soldOut ? 'text-yovel-muted' : '' }}">Rp {{ number_format($product->product_price, 0, ',', '.') }}</span>
                                @unless ($soldOut)
                                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#37352F] text-white shadow-sm transition-colors group-hover:bg-black">
            ...
            <div x-show="searchQuery.length > 0 && !$el.previousElementSibling.innerText.toLowerCase().includes(searchQuery.toLowerCase())" x-cloak class="col-span-full py-12 text-center text-yovel-muted text-sm mt-8">
    ...
    {{-- Toast --}}
    <div x-show="toastOn" x-cloak role="status"
