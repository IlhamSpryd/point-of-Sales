<section>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-form-label for="update_password_current_password">{{ __('Current Password') }}</x-form-label>
        ...
        <div>
            <x-form-label for="update_password_password">{{ __('New Password') }}</x-form-label>
        ...
        <div>
            <x-form-label for="update_password_password_confirmation">{{ __('Confirm Password') }}</x-form-label>
        ...
        <div class="flex items-center gap-4">
            <button type="submit" class="inline-flex items-center justify-center px-4 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-sm font-medium rounded-lg hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                <span class="material-symbols-rounded mr-2">key</span> {{ __('Update Password') }}
            </button>
