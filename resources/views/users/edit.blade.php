<x-app-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h4 class="text-xl font-bold text-yovel-ink tracking-tight">Edit Pengguna</h4>
            <p class="text-sm font-medium text-[#787774] mt-1">Ubah data pengguna sistem</p>
        </div>
        <a href="{{ route('users.index') }}" class="flex items-center gap-2 px-4 py-2 bg-white hover:bg-[#F7F7F5] border border-[#E9E9E7] text-yovel-ink text-sm font-medium rounded-xl transition-all duration-200 shadow-sm active:scale-95" wire:navigate>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[#E9E9E7]">
                <div>
                    ...
                    <div class="relative">
                        <select id="role_id" name="role_id" 
                                class="form-input w-full px-4 py-2.5 rounded-xl border border-[#E9E9E7] bg-white text-yovel-ink appearance-none focus:outline-none focus:ring-2 focus:ring-[#37352F] focus:border-yovel-ink transition-all duration-200 shadow-sm {{ $errors->has('role_id') ? 'input-error' : '' }}" required>
                ...
                <div class="flex items-center pt-8">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-[#E9E9E7] text-yovel-ink focus:ring-[#37352F] transition-colors">
                        <span class="text-sm font-medium text-yovel-ink">Karyawan Aktif</span>
                    </label>
                </div>
            </div>
