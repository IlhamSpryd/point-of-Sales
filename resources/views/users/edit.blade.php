<x-app-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h4 class="text-xl font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">Ubah Pengguna <!-- Standarisasi bahasa UjiKom --></h4>
            <p class="text-sm font-medium text-zinc-500 mt-1">Perbarui detail untuk {{ $user->name }} <!-- Standarisasi bahasa UjiKom --></p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-zinc-100 dark:border-zinc-800 overflow-hidden w-full">
        <form action="{{ route('users.update', $user->id) }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true" class="p-6 md:p-8 space-y-6">
            ...
            <div class="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                <div class="mb-4">
                    <h5 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Perbarui Kata Sandi <!-- Standarisasi bahasa UjiKom --></h5>
                    <p class="text-xs text-zinc-500 mt-1">Biarkan kosong jika Anda tidak ingin mengubah kata sandi. <!-- Standarisasi bahasa UjiKom --></p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-form-label for="password">Kata Sandi Baru <!-- Standarisasi bahasa UjiKom --></x-form-label>
                    ...
                    <div>
                        <x-form-label for="password_confirmation">Konfirmasi Kata Sandi Baru <!-- Standarisasi bahasa UjiKom --></x-form-label>
            ...
            <div class="pt-6 border-t border-zinc-100 dark:border-zinc-800 flex justify-end">
                ...
                <x-button type="submit" variant="primary" :disabled="false" x-bind:disabled="submitting" x-bind:class="submitting ? 'opacity-60 cursor-not-allowed' : ''">
                    <span x-show="!submitting">
                        <span class="material-symbols-rounded">save</span> Perbarui Pengguna <!-- Standarisasi bahasa UjiKom -->
                    </span>
